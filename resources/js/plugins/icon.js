        ...
        for (var prop in obj) {
            if (obj.hasOwnProperty(prop)) {
                if (
                    ...
                    extended[prop] = extendIconPicker(
                        extended[prop],
                        obj[prop]
                    );
                } else {
...
var IconPicker = {
    ...
    Run: function (theButton, theCallback) {
        ...
        if (arguments && arguments.length <= 2) {
            ...
            if (ipButtons && ipButtons.length > 0) {
                for (var i = 0; i < ipButtons.length; i++) {
                    ...
                    ipButton.addEventListener("click", function () {
                        ...
                        if (
                            !showAllButton ||
                            (showAllButton && showAllButton.length < 1)
                        ) {
                        ...
                        if (
                            !cancelButton ||
                            (cancelButton && cancelButton.length < 1)
                        ) {
                        ...
                        if (
                            !searchPlaceholder ||
                            (searchPlaceholder && searchPlaceholder.length < 1)
                        ) {
                            searchPlaceholder =
                                ipDefaultOptions.searchPlaceholder;
                        }
                        ...
                        if (!jsonUrl) {
                            ipConsoleError(
                                'You have to set the path of IconPicker JSON file to "jsonUrl" option. \n\nVisit to learn how: ' +
                                    ipGithubUrl
                            );
                            return false;
                        }
                        ...
                        if (checkInput.length <= 0) {
                            ipConsoleError(
                                'You must define your Input element with it\'s ID or Class Name to your Button element data attribute. \n\nExample: \ndata-iconpicker-input="#MyIconInput" or \ndata-iconpicker-input=".my-icon-input" \n\nVisit to learn how: ' +
                                    ipGithubUrl
                            );
                            return false;
                        }
                        ...
                        var checkPreviewIcon = document.querySelectorAll(
                            previewElement
                        );
                        if (checkPreviewIcon.length <= 0) {
                            ipConsoleLog(
                                'You can define your Preview Icon element with it\'s ID or Class Name to your Button element data attribute. \n\nExample: \ndata-iconpicker-preview="i#MyIconElement" or \ndata-iconpicker-preview="i.my-icon-element" \n\nVisit to learn how: ' +
                                    ipGithubUrl
                            );
                        }
            ...
            ipConsoleError(
                "You have to call the IconPicker with an Element(Button or etc.) Class or ID. \n\nYou can also find the other required data attributes in the Documentation. \n\nVisit: " +
                    ipGithubUrl
            );
            return false;
        }
        ...
        var getIconListXmlHttpRequest = function (
            jsonUrl,
            buttonShowAll,
            buttonCancel,
            searchPlaceholder,
            ...
            if (navigator.userAgent.toLowerCase().indexOf("chrome") > -1) {
                ...
                if (window.location.protocol.indexOf("http") <= -1) {
                    ipConsoleLog(
                        "Chrome Browser blocked this request by CORS policy."
                    );
                    return false;
                }
            }
        ...
        var appendIconListToBody = function (
            data,
            buttonShowAll,
            buttonCancel,
            searchPlaceholder,
            ...
            for (var key in jsonData) {
                if (jsonData.hasOwnProperty(key)) {
                    ...
                    var id = iconModel
                        ? `@click="$wire.set('${iconModel}', '${forObj}')"`
                        : "";
                    var icon =
                        '<i data-search="' +
                        forObj +
            ...
            var loadingIndicator =
                '<div id="IconPickerLoading"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="60" height="60" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid"><g transform="translate(25 50)"><circle cx="0" cy="0" r="9" fill="#1e1e1e" transform="scale(0.24 0.24)"><animateTransform attributeName="transform" type="scale" begin="-0.2666s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g><g transform="translate(50 50)"><circle cx="0" cy="0" r="9" fill="#1e1e1e" transform="scale(0.00153 0.00153)"><animateTransform attributeName="transform" type="scale" begin="-0.1333s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g><g transform="translate(75 50)"><circle cx="0" cy="0" r="9" fill="#1e1e1e" transform="scale(0.3 0.3)"><animateTransform attributeName="transform" type="scale" begin="0s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g></svg></div>';

            // icons wrap
            var iconWrap =
                '<div class="ip-icons-content" style="border-radius:' +
                borderRadius +
                ';">' +
                '<div class="ip-icons-search"><input id="IconPickerSearch" type="text" spellcheck="false" autocomplete="off" placeholder="' +
                searchPlaceholder +
                '" style="border-radius:' +
                borderRadius +
                ';" /><i class="placeholder-icon fas fa-search"></i></div>' +
                '<div class="ip-icons-search-results"></div>' +
                '<div class="ip-icons-area">' +
                loadingIndicator +
                icons +
                '<a class="ip-show-all-icons" style="border-radius:' +
                borderRadius +
                ';">' +
                buttonShowAll +
                "</a>" +
                "</div>" +
                '<div class="ip-icons-footer"><a class="cancel" style="border-radius:' +
                borderRadius +
                ';">' +
                buttonCancel +
                "</a></div>" +
            ...
            // body
            var docBody = document.getElementById(containerModal);
            ...
            var showAllButtonElm = document
                .getElementById(IconPickerModal.id)
                .getElementsByClassName("ip-show-all-icons")[0];
            showAllButtonElm.addEventListener(
                "click",
                function () {
            ...
            var cancelButtonElm = document
                .getElementById(IconPickerModal.id)
                .getElementsByClassName("cancel")[0];
            cancelButtonElm.addEventListener(
                "click",
                function () {
                    removeIpElement(400);
                },
            ...
            searchInputElm.addEventListener(
                ...
                function (e) {
                    ...
                    // define icons areas
                    var firstIconsArea = document
                        .getElementById(IconPickerModal.id)
                        .getElementsByClassName("ip-icons-area")[0];
                    var searchResultArea = document
                        .getElementById(IconPickerModal.id)
                        .getElementsByClassName("ip-icons-search-results")[0];
                    ...
                    if (!spaceOrBackspace && searchVal.length > 0) {
                        ...
                        if (tempIcons.length < 1) {
                            ...
                            if (
                                !noResultsText ||
                                (noResultsText && noResultsText.length < 1)
                            ) {
                                noResultsText = ipDefaultOptions.noResultsFound;
                            }
                            var noResultElm =
                                '<p class="ip-no-results-found">' +
                                noResultsText +
                                "</p>";
            ...
            var eachIconEventListener = function (firstOrSearch) {
                ...
                if (firstOrSearch === "first") {
                    // first
                    eachIconElm = document
                        .getElementById(IconPickerModal.id)
                        .getElementsByClassName("first-icon");
                } else if (firstOrSearch === "search") {
                    // search
                    eachIconElm = document
                        .getElementById(IconPickerModal.id)
                        .getElementsByClassName("search-icon");
                }
                // define icons off
                ...
                for (var i = 0; i < eachIconElm.length; i++) {
                    ...
                    singleIconElm.addEventListener(
                        "click",
                        function (e) {
                            e.preventDefault();
                            ...
                            for (var i = 0; i < inputElm.length; i++) {
                                var getTagName = inputElm[i].tagName
                                    .toString()
                                    .toLowerCase();
...
module.exports = { Init: IconPicker.Init, Run: IconPicker.Run };
