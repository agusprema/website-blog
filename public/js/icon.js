...
var ipNewOptions;
var ipGithubUrl = "https://github.com/furcan/IconPicker"; // IconPicker: Default Options off
// IconPicker: Extend Options on

var extendIconPicker = function extendIconPicker() {
  var extended = {};
  var deep = false;
  var i = 0;
...
var IconPicker = {
  // init
  Init: function Init(ipUserOptions) {
    ipNewOptions = extendIconPicker(true, ipDefaultOptions, ipUserOptions);
  },
  // run
  Run: function Run(theButton, theCallback) {
    // IconPicker: Console Error Function on
    var ipConsoleError = function ipConsoleError(errorMessage) {
    ...
    var ipConsoleLog = function ipConsoleLog(errorMessage) {
    ...
    if (arguments && arguments.length <= 2) {
      // query selector
      var ipButtons = document.querySelectorAll(theButton); // if button exist on the document
      ...
      if (ipButtons && ipButtons.length > 0) {
        for (var i = 0; i < ipButtons.length; i++) {
          ...
          ipButton.addEventListener("click", function () {
            ...
            var containerModal = this.dataset.iconContainer;
            var iconModel = this.dataset.iconModel;
            ...
            getIconListXmlHttpRequest(jsonUrl, showAllButton, cancelButton, searchPlaceholder, borderRadius, inputElement, previewElement, containerModal, iconModel, theCallback);
          }); // IconPicker: Button Listeners -> Send XMLHttpRequest off
        }
      } // not exist
      // else {
      //     ipConsoleError(
      //         'You called the IconPicker with "' +
      //             theButton +
    ...
    var getIconListXmlHttpRequest = function getIconListXmlHttpRequest(jsonUrl, buttonShowAll, buttonCancel, searchPlaceholder, borderRadius, inputElement, previewElement, containerModal, iconModel, theCallback) {
      // if chrome browser
      ...
      var ipElement = document.getElementById("IconPickerModal"); // if modal element doesn't exist on document send XMLHttpRequest
        ...
        xmlHttp.onreadystatechange = function () {
          if (this.readyState === 4) {
            if (this.status === 200) {
              ...
              appendIconListToBody(data, buttonShowAll, buttonCancel, searchPlaceholder, borderRadius, inputElement, previewElement, containerModal, iconModel, theCallback);
            } else {
    ...
    var appendIconListToBody = function appendIconListToBody(data, buttonShowAll, buttonCancel, searchPlaceholder, borderRadius, inputElement, previewElement, containerModal, iconModel, theCallback) {
      // data
      ...
      for (var key in jsonData) {
        if (jsonData.hasOwnProperty(key)) {
          ...
          var id = iconModel ? "@click=\"$wire.set('".concat(iconModel, "', '").concat(forObj, "')\"") : "";
          var icon = '<i data-search="' + forObj + '" data-class="' + forObj + '" class="first-icon select-icon ' + forObj + '" ' + id + "></i>";
          icons += icon;
        }
      } // loading indicator


      var loadingIndicator = '<div id="IconPickerLoading"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="60" height="60" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid"><g transform="translate(25 50)"><circle cx="0" cy="0" r="9" fill="#1e1e1e" transform="scale(0.24 0.24)"><animateTransform attributeName="transform" type="scale" begin="-0.2666s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g><g transform="translate(50 50)"><circle cx="0" cy="0" r="9" fill="#1e1e1e" transform="scale(0.00153 0.00153)"><animateTransform attributeName="transform" type="scale" begin="-0.1333s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g><g transform="translate(75 50)"><circle cx="0" cy="0" r="9" fill="#1e1e1e" transform="scale(0.3 0.3)"><animateTransform attributeName="transform" type="scale" begin="0s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g></svg></div>'; // icons wrap

      var iconWrap = '<div class="ip-icons-content" style="border-radius:' + borderRadius + ';">' + '<div class="ip-icons-search"><input id="IconPickerSearch" type="text" spellcheck="false" autocomplete="off" placeholder="' + searchPlaceholder + '" style="border-radius:' + borderRadius + ';" /><i class="placeholder-icon fas fa-search"></i></div>' + '<div class="ip-icons-search-results"></div>' + '<div class="ip-icons-area">' + loadingIndicator + icons + '<a class="ip-show-all-icons" style="border-radius:' + borderRadius + ';">' + buttonShowAll + "</a>" + "</div>" + '<div class="ip-icons-footer"><a class="cancel" style="border-radius:' + borderRadius + ';">' + buttonCancel + "</a></div>" + "</div>"; // create the modal element

      var IconPickerModal = document.createElement("div");
      IconPickerModal.id = "IconPickerModal";
      IconPickerModal.innerHTML = iconWrap; // body

      var docBody = document.getElementById(containerModal); // append to body

      docBody.appendChild(IconPickerModal); // get the modal element

      var ipElement = document.getElementById(IconPickerModal.id); // modal element display css

      ipElement.style.display = "block"; // heights

      var ipHeight = parseInt(ipElement.offsetHeight);
      var winHeight = parseInt(window.innerHeight); // modal element position top css on
      ...
      ipElement.style.top = totalTopPos + "px"; // modal element position top css off
      // add css animation class to modal

      ipElement.classList.add("animate"); // remove loading indicator on
      ...
      }, 900); // remove loading indicator off
      ...
      }, false); // show all button click listener off
      // close and remove all on

      var removeIpElement = function removeIpElement(delay) {
      ...
      cancelButtonElm.addEventListener("click", function () {
        removeIpElement(400);
      }, false); // cancel button click listener off
      ...
      searchInputElm.addEventListener("keyup", function (e) {
        ...
        var eCode = e.code.toString().toLowerCase(); // check space and backspace keyup
        ...
        var searchVal = this.value.toString().toLowerCase(); // define icons areas

        var firstIconsArea = document.getElementById(IconPickerModal.id).getElementsByClassName("ip-icons-area")[0];
        var searchResultArea = document.getElementById(IconPickerModal.id).getElementsByClassName("ip-icons-search-results")[0]; // clear old results

        searchResultArea.innerHTML = ""; // (if not space or backspace) and (if typed at least one char) able to search
        ...
        if (!spaceOrBackspace && searchVal.length > 0) {
          ...
          for (var key in jsonData) {
            if (jsonData.hasOwnProperty(key)) {
              var forObj = jsonData[key]; // if there is results create them
          ...
          tempResults.innerHTML = tempIcons; // no results found on
          ...
          searchResultArea.appendChild(tempResults); // event listener for each temp icon
      ...
      }, false); // search input keyup listener off
      // each icon click listener on

      var eachIconEventListener = function eachIconEventListener(firstOrSearch) {
        var inputElm = document.querySelectorAll(inputElement);
        var previewElm = document.querySelectorAll(previewElement); // define icons on
        ...
        for (var i = 0; i < eachIconElm.length; i++) {
          ...
          singleIconElm.addEventListener("click", function (e) {
            e.preventDefault();
            var iconClassName = this.dataset["class"]; // each input value on

            for (var i = 0; i < inputElm.length; i++) {
              var getTagName = inputElm[i].tagName.toString().toLowerCase(); // if an input or textarea element
...
/* harmony import */ var _plugins_icon__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./plugins/icon */ "./resources/js/plugins/icon.js");
/* harmony import */ var _plugins_icon__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_plugins_icon__WEBPACK_IMPORTED_MODULE_0__);

_plugins_icon__WEBPACK_IMPORTED_MODULE_0___default().Init({
  jsonUrl: "../json/icon.json"
});
_plugins_icon__WEBPACK_IMPORTED_MODULE_0___default().Run("#GetIconPicker");
})();
